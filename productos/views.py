...
def convertir_categoria(valor):
    ...
    if categoria not in {
        "mujer", "hombre", "unisex", "arabes", "sets"
    }:
        raise ValueError("Seleccioná una categoría válida para perfumes")
    return categoria
...
def index(request):
    ...
    return render(
        ...
        'index.html',
        {
            ...
            'hay_kits': productos.filter(es_kit=True).exists(),
...
def crear_pedido(request):
    ...
    try:
        ...
        for item in items:
            ...
            opciones_validas = producto.lista_variantes
...
def editar_producto(request, id):
    ...
    if request.method == "POST":
        ...
        producto.es_kit = tipo_publicacion == "kit"
        producto.variantes = request.POST.get(
            "variantes", ""
        ).strip()
